...
def main():
    ...
    save_folder = 'SaveResults'
    try:
        os.mkdir(save_folder)
        inx_saveResults = 1
    except:
        print('already have the folder')
        files_name = os.listdir(save_folder)
        inx_saveResults = (len(files_name)/2)+1

    imgs_out = []
    imgs = []
    fileTXT = open(os.path.join(save_folder,"SaveTimesResult_%d.mp4"%(inx_saveResults)),"w")
    while(cap.isOpened()):
        tic = time.time()
        ret, img = cap.read()
        ...
        imgs.append(img)
        fileTXT.writelines("{},    {}\n".format(datetime.datetime.now(), posture))
...
#        print("posture is", posture, ": fps is %.2f"%(1/(time.time()-tic)))
    ...
    out = cv2.VideoWriter(os.path.join(save_folder,"RawVideo_%d.mp4"%(inx_saveResults)),
    cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (wid, hih))
    ...
    out = 0
    out = cv2.VideoWriter(os.path.join(save_folder,"ResultVideo_%d.mp4"%(inx_saveResults)),
    cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (wid, hih))
    
    for i in range(len(imgs_out)):
        out.write(imgs_out[i])
    out.release()
